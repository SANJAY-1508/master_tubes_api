...
if (isset($obj->search_text)) {
    ...
    $select_cols = "`p`.`id`, `p`.`product_id`, `p`.`product_name`, `p`.`product_img`, `p`.`product_code`, 
                    `p`.`unit_id`, `p`.`category_id`, `p`.`product_details`, `p`.`product_price`, 
                    `p`.`product_with_discount_price`, `p`.`product_stock`, `p`.`product_disc_amt`, 
                    `p`.`discount_lock`, `p`.`new_arrival`, `p`.`top_selling`, `p`.`status`, 
                    `p`.`created_at`, `u`.`unit_name`, `c`.`category_name`";
    ...
    $sql = "SELECT $select_cols FROM `product` p 
            ...
            WHERE p.deleted_at = 0 AND p.status = 0 
...
else if (isset($obj->product_name) && isset($obj->product_code) && isset($obj->unit_id) && 
         isset($obj->category_id) && isset($obj->user_id) && isset($obj->created_name)) {
    ...
    $top_selling      = isset($obj->top_selling) ? intval($obj->top_selling) : 0; // ← NEW FIELD

    if (!empty($product_name) && !empty($product_code) && !empty($unit_id) && 
        !empty($category_id) && !empty($user_id) && !empty($created_name)) {
        ...
        if (isset($obj->product_id)) {
            ...
            if ($edit_id) {
                ...
                if ($check->num_rows > 0) {
                    ...
                    if (!empty($product_img)) {
                        ...
                        $updateProduct = "UPDATE `product` SET 
                            ...
                            `product_img`='$savedImageName', 
                            `product_details`='$product_details', 
                            `product_price`=$product_price, 
                            `product_with_discount_price`=$product_with_discount_price, 
                            `product_stock`=$product_stock, 
                            `product_disc_amt`=$product_disc_amt, 
                            `discount_lock`=$discount_lock, 
                            `new_arrival`=$new_arrival, 
                            `top_selling`=$top_selling,          /* ← NEW */
                            `status`=$status, 
                            `created_by`='$user_id', 
                ...
                $output["head"]["code"] = 400;
                $output["head"]["msg"] = "Invalid product_id.";
            }
        } 
        else {
            // ==================== CREATE ====================
            ...
            if ($check->num_rows == 0) {
                ...
                if (!empty($product_img)) {
                    ...
                    $createProduct = "INSERT INTO `product` 
                        (`product_name`, `product_code`, `unit_id`, `category_id`, `product_img`, `product_details`, 
                         `product_price`, `product_with_discount_price`, `product_stock`, `product_disc_amt`, 
                         `discount_lock`, `new_arrival`, `top_selling`, `status`, `created_by`, `created_name`, `created_at`) 
                        VALUES 
                        ('$product_name', '$product_code', '$unit_id', '$category_id', '$savedImageName', '$product_details', 
                         $product_price, $product_with_discount_price, $product_stock, $product_disc_amt, 
                         $discount_lock, $new_arrival, $top_selling, $status, '$user_id', '$created_name', '$timestamp')";
                } else {
                    $createProduct = "INSERT INTO `product` 
                        (`product_name`, `product_code`, `unit_id`, `category_id`, `product_details`, 
                         `product_price`, `product_with_discount_price`, `product_stock`, `product_disc_amt`, 
                         `discount_lock`, `new_arrival`, `top_selling`, `status`, `created_by`, `created_name`, `created_at`) 
                        VALUES 
                        ('$product_name', '$product_code', '$unit_id', '$category_id', '$product_details', 
    ...
    if (!empty($delete_product_id)) {
        ...
        if ($conn->query($deleteproduct) === true && $conn->affected_rows > 0) {
            ...
            $output["head"]["code"] = 400;
            $output["head"]["msg"] = "Failed to delete. Product may not exist.";
        }
    } else {
        $output["head"]["code"] = 400;
        $output["head"]["msg"] = "Please provide product_id.";
    }
} 
else {
    $output["head"]["code"] = 400;
